 ...
 * Waits for the AudioContext to actually be running before scheduling
 * oscillators — starting them while the context is still 'suspended'
 * silently drops the sound in browsers with strict autoplay policies.
 */
export const playNotificationSound = (level: string = 'info') => {
  ...
  if (ctx.state === 'suspended') {
    ctx.resume().then(emit).catch(err => console.warn('[Audio] Unlock failed:', err));
  } else {
    emit();
  }
};